...
SHELL_COMMANDS_FILE = PROJECT_ROOT / 'shell_commands.json'
DETECT_KEYBOARD_SCRIPT = PROJECT_ROOT / 'detect_keyboard.py'
CHECK_KEYBOARD_USB_SCRIPT = PROJECT_ROOT / 'scripts' / 'check_keyboard_usb.sh'
...
def load_shell_commands():
    ...
    if SHELL_COMMANDS_FILE.exists():
        try:
            with SHELL_COMMANDS_FILE.open('r') as f:
...
def save_shell_commands():
    ...
    try:
        with SHELL_COMMANDS_FILE.open('w') as f:
...
class ShellCommandListener:
    ...
    def execute_command(self, command):
        ...
        def run():
            try:
                subprocess.run(command, shell=True, capture_output=True, text=True, cwd=PROJECT_ROOT)
            except Exception as e:
...
def get_device_info():
    ...
    try:
        result = subprocess.run(
            [sys.executable, str(DETECT_KEYBOARD_SCRIPT)],
            capture_output=True,
            text=True,
            timeout=5,
            cwd=PROJECT_ROOT
...
def list_presets():
    ...
    try:
        ...
        preset_files = [path.stem for path in PRESETS_DIR.glob('*.yaml')]
